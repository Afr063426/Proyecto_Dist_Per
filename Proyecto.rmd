...
summary(evd::fpot(x=c(12,31,213,12,543,5,56),threshold = 5,model="gpd",std.err=FALSE))
...
norm_L_1_GPD_KDE <- function(X,eps=0,iter=1000,lower,upper){
  ...
  fn <- function(u){
    ...
    integral <- integrate(function(x) abs(kde(x)/(F_kde_u)-evd::dgpd(x-u,loc=0,scale=GPD_params[2],shape= GPD_params[1],log=FALSE)),lower = 0,upper=Inf)$value
...
parameters <- function(X,eps=0,iter=1000,lower,upper){
  ...
  return(list(shape=GPD_params[1],scale=GPD_params[2],u=p$par*1000000,error=p$value))
}
...
create_dplot <- function(X,list_params,cols="red",title,u){
        ...
        plot<- ggplot(,)+ scale_x_continuous(breaks = seq(0,M,length.out=4),labels = round(seq(0,M/1000,length.out=4),0),limits=c(0,M)) +stat_function(size=2,fun = evd::dgpd,args=list(shape=list_params$shape,loc=list_params$loc,scale=list_params$scale),colour=cols)+theme(
                text =  element_text(size = 17),
                legend.position = "none",
                plot.caption = element_text(hjust = 0),
                panel.spacing = unit(0.65, "cm"),
...
estimation_nakamura <- data.frame("Evento"=character(),"Mínimo"=numeric(),"Máximo"=numeric(),shape = numeric(),scale=numeric(),u=numeric(),error=numeric())
...
for (i in 1:length(events)){
  ...
  estimation_nakamura <- rbind(estimation_nakamura, data.frame("Evento"=events[i],"Mínimo"=lower_upper_graph[1],"Máximo"=lower_upper_graph[2],shape = estimation$shape,scale=estimation$scale,u=estimation$u,error=estimation$error))
}
...
for (i in 1:length(events)){
  ...
  assign(paste("qq_plot",events[i],sep="_"),create_qqplot(title=events[i],cols=cols[i], data=X,distribution="gpd",list_params=list(shape=estimation_nakamura$shape[i],scale = estimation_nakamura$scale[i],loc = 0)))
}
qq_plot_Terremoto
...
for (i in 1:length(events)){
  X <- t(base_de_datos%>%filter(Type == events[i])%>%select(`NORMALISED LOSS VALUE (2017)`))
  X[is.na(X)]<-0
  assign(paste("density_plot",events[i],sep="_"),create_dplot(u=estimation_nakamura$u[i],title=events[i],cols=cols[i],X=X,list_params=list(shape=estimation_nakamura$shape[i],scale = estimation_nakamura$scale[i],loc = 0)))
}
density_plot_Ciclón
