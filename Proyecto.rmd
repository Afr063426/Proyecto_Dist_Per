...
base_de_datos <- base_de_datos %>%dplyr::select(-c(`REGION / TOWN`,FinYeae))
...
create_dplot <- function(X,list_params,cols="red",title,u){
        ...
        #print(counting)
        #print(number)
        plot<- ggplot(,)+ xlim(0,M) +stat_function(size=2,fun = evd::dgpd,args=list(shape=list_params$shape,loc=list_params$loc,scale=list_params$scale),colour=cols)+theme(
                text =  element_text(size = 17),
                legend.position = "none",
                plot.caption = element_text(hjust = 0),
                panel.spacing = unit(0.65, "cm"),
...
for (i in 1:length(events)){
  X <- t(base_de_datos%>%filter(Type == events[i])%>%dplyr::select(`NORMALISED LOSS VALUE (2017)`))
  X[is.na(X)]<-0
...
#Max of each event 
for (i in 1:length(events)){
  X <- (base_de_datos%>%filter(Type == events[i])%>%dplyr::select(`NORMALISED LOSS VALUE (2017)`))
...
for (i in 1:length(events)){
  ...
  X <- t(data_model%>%dplyr::select(`NORMALISED LOSS VALUE (2017)`))
  X[is.na(X)] <- 0 
  #print(length(X))
